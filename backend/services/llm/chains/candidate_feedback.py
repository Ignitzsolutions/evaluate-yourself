...
Uses provider-neutral OpenAI-compatible analysis with LangChain structured output support.
"""
...
def _get_langchain_client():
    """Return LangChain ChatOpenAI client from env."""
...
def _generate_structured_feedback(
    transcript_str: str, interview_type: str, duration_minutes: int
) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if "404" in msg or "Resource not found" in msg:
            print(
                "⚠️ Structured feedback deployment not found; using deterministic fallback feedback."
            )
        else:
...
def _generate_legacy_feedback(
    transcript_str: str, interview_type: str, duration_minutes: int, active_scores: Dict[str, int]
) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if "404" in msg or "Resource not found" in msg:
            print(
                "⚠️ Candidate feedback deployment not found; using deterministic fallback feedback."
            )
        else:
...
def _normalize_feedback_contract(
    feedback: Dict[str, Any], scores: Dict[str, int], transcript_str: str
) -> Dict[str, Any]:
    ...
    while len(improvements) < 2:
        improvements.append(
            base["areas_for_improvement"][len(improvements) % len(base["areas_for_improvement"])]
        )
    while len(tips) < 3:
        tips.append(
            base["tips_for_next_interview"][len(tips) % len(base["tips_for_next_interview"])]
        )
    ...
    return {
        ...
        "communication_feedback": str(
            feedback.get("communication_feedback") or base["communication_feedback"]
        ),
...
def _generate_fallback_feedback(scores: Dict[str, int], transcript_str: str = "") -> Dict[str, Any]:
    ...
    if overall >= 80:
        ...
        strengths = [
            ...
            "Reasonable structure in answers",
