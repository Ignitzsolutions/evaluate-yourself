...
class AuthMiddleware:
    ...
    @staticmethod
    def extract_token(
        authorization_header: Optional[str], query_token: Optional[str]
    ) -> Optional[str]:
    ...
    def validate_and_extract(
        self, authorization_header: Optional[str], query_token: Optional[str]
        ...
        if not session_id or not tenant_id:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Incomplete token claims"
            )
