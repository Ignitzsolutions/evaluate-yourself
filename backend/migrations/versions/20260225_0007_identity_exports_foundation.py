...
def upgrade() -> None:
    ...
    if not _table_exists(bind, "auth_identities"):
        ...
        op.create_index(
            "ix_auth_identities_external_id", "auth_identities", ["external_id"], unique=False
        )
        op.create_index(
            "ix_auth_identities_legacy_provider_user_id",
            "auth_identities",
    ...
    if not _table_exists(bind, "candidate_profiles"):
        op.create_table(
            ...
            sa.Column(
                "consent_data_use", sa.Boolean(), nullable=False, server_default=sa.text("false")
            ),
            sa.Column(
                "consent_contact", sa.Boolean(), nullable=False, server_default=sa.text("false")
            ),
        ...
        op.create_index(
            "ix_candidate_profiles_primary_stream",
            "candidate_profiles",
            ["primary_stream"],
            unique=False,
        )
        op.create_index(
            "ix_candidate_profiles_state_code", "candidate_profiles", ["state_code"], unique=False
        )
    ...
    if not _table_exists(bind, "candidate_profile_versions"):
        ...
        op.create_index(
            "ix_candidate_profile_versions_user_id",
            "candidate_profile_versions",
            ["user_id"],
            unique=False,
        )
    ...
    if not _table_exists(bind, "admin_export_jobs"):
        ...
        op.create_index(
            "ix_admin_export_jobs_created_by_user_id",
            "admin_export_jobs",
            ["created_by_user_id"],
            unique=False,
        )
        op.create_index(
            "ix_admin_export_jobs_export_type", "admin_export_jobs", ["export_type"], unique=False
        )
        op.create_index(
            "ix_admin_export_jobs_status", "admin_export_jobs", ["status"], unique=False
        )
    ...
    if _table_exists(bind, "interview_sessions") and _column_exists(
        bind, "interview_sessions", "user_id"
    ):
    ...
    if _table_exists(bind, "user_entitlements") and _column_exists(
        bind, "user_entitlements", "user_id"
    ):
        if not _index_exists(bind, "user_entitlements", "ix_user_entitlements_user_id"):
            op.create_index(
                "ix_user_entitlements_user_id", "user_entitlements", ["user_id"], unique=False
            )
    if _table_exists(bind, "interview_gaze_events") and _column_exists(
        bind, "interview_gaze_events", "user_id"
    ):
        if not _index_exists(bind, "interview_gaze_events", "ix_interview_gaze_events_user_id"):
            op.create_index(
                "ix_interview_gaze_events_user_id",
