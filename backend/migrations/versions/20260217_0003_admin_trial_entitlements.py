...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_trial_codes_redeemed_by_clerk_user_id",
        "trial_codes",
        ["redeemed_by_clerk_user_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "duration_minutes_effective", sa.Integer(), nullable=False, server_default=sa.text("5")
        ),
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default=sa.text("true")),
        sa.Column(
            "starts_at", sa.DateTime(timezone=True), server_default=sa.text("CURRENT_TIMESTAMP")
        ),
    ...
    op.create_index(
        "ix_user_entitlements_clerk_user_id", "user_entitlements", ["clerk_user_id"], unique=False
    )
    op.create_index(
        "ix_user_entitlements_source_id", "user_entitlements", ["source_id"], unique=False
    )
    op.create_index(
