...
def main():
    with engine.connect() as c:
        r = c.execute(
            text(
                "SELECT id, clerk_user_id, email, full_name, created_at FROM users ORDER BY created_at DESC LIMIT 10"
            )
        )
        rows = r.fetchall()
    if not rows:
        print(
            "No users in DB yet. Log in via Clerk and open Dashboard so /api/me creates the user."
        )
        return
    print(f"Found {len(rows)} user(s) in DB:")
    for row in rows:
        uid, cid, em, _name, _created = (
            row[0],
            row[1] or "",
            row[2] or "",
            row[3] or "",
